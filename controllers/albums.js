...
// create  album
 const schema = joi.object({name:joi.string(),year:joi.number().min(1900).max(2023).required(),
...
const create = async (req,res)=>{
   ...
   if(error1.error){
     ...
     const music = req.body.songs;
     const songdata = music.map((song)=>{
       song.albumId = album.id;
       return song;
     });
     console.log(songdata);
     await songs.bulkCreate(songdata,{transaction:a});
         await a.commit();
              res.send(album);
          }catch(err){
           await a.rollback()
...
const err = joi.object().keys({
      ...
      const update = (req, res) => {
        ...
        albums.update({name:req.body.name,year:req.body.year},{ where: { id: albumId } }).then(
          ([RowsUpdated]) => {
            if (RowsUpdated === 0) {
